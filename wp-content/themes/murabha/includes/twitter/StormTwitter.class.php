...
class StormTwitter {
  ...
  function getTweets($screenname = false,$count = 20,$options2 = false) {
    ...
    if ($options2 === false || !is_array($options2)) {
      $options2 = $default_options;
    } else {
      $options2 = array_merge($default_options, $options2);
    }

    if ($screenname === false || $screenname === 20) $screenname = $this->defaults['screenname'];

    $result = $this->checkValidCache($screenname,$options2);
    ...
    $result = $this->oauthGetTweets($screenname,$options2);
  ...
  private function getOptionsHash($options2) {
    $hash = md5(serialize($options2));
    return $hash;
  }

  private function checkValidCache($screenname,$options2) {
    ...
    if (is_file($file)) {
      ...
      $cachename = $screenname."-".$this->getOptionsHash($options2);
  ...
  private function oauthGetTweets($screenname,$options2) {
    ...
    $options2 = array_merge($options2, array('screen_name' => $screenname, 'count' => 20));
    ...
    $result = $connection->get('statuses/user_timeline', $options2);
